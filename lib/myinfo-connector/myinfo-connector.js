...
export default class MyInfoConnector {
  ...
  load = function (config) {
    ...
    if (!config.CLIENT_PRIVATE_SIGNING_KEY) {
      throw new Error(constant.ERROR_CONFIGURATION_CLIENT_PRIVATE_SIGNING_KEY_NOT_FOUND);
    } else {
      CONFIG.CLIENT_PRIVATE_SIGNING_KEY = fs.readFileSync(
        config.CLIENT_PRIVATE_SIGNING_KEY,
        "utf8"
      );
      this.CLIENT_PRIVATE_SIGNING_KEY = CONFIG.CLIENT_PRIVATE_SIGNING_KEY;
    }
    if (!config.CLIENT_PRIVATE_ENCRYPTION_KEY) {
      throw new Error(constant.ERROR_CONFIGURATION_CLIENT_PRIVATE_ENCRYPTION_KEY_NOT_FOUND);
    } else {
      CONFIG.CLIENT_PRIVATE_ENCRYPTION_KEY = fs.readFileSync(
        config.CLIENT_PRIVATE_ENCRYPTION_KEY,
        "utf8"
      );
      this.CLIENT_PRIVATE_ENCRYPTION_KEY = CONFIG.CLIENT_PRIVATE_ENCRYPTION_KEY;
    }
  ...
  getMyInfoPersonData = async function (authCode, codeVerifier) {
    ...
    try {
      let sessionPopKeyPair = await this.securityHelper.generateSessionKeyPair();
  ...
  callTokenAPI = async function (authCode, privateSigningKey, codeVerifier, sessionPopKeyPair) {
    ...
    // invoke Token API
    let tokenURL =
      CONFIG.USE_PROXY && CONFIG.USE_PROXY == "Y" ? CONFIG.PROXY_TOKEN_URL : CONFIG.TOKEN_URL;
  ...
  callCorporateVcAPI = async function (sub, accessToken, sessionPopKeyPair) {
    ...
    let did = CONFIG.VC_WALLET_ADDRESS;
    ...
    let signedCodeChallenge = MyInfoVcVerifier.ethereumSign(privateKey, codeChallenge);
    ...
    logger.info("Body for Corporate VC Get Credential API:", JSON.stringify(body));
  ...
  callCorporateVcCodeChallengeAPI = async function (did, accessToken, sessionPopKeyPair) {
    ...
    let result = await requestHandler.getHttpsResponse(method, urlLink, headers, body, null);
    let codeChallengeResult = result.data;
    logger.debug("Corporate VC Code Challenge API Response (JWE): ", codeChallengeResult);
    let jws = await this.securityHelper.decryptJWEWithKey(
      codeChallengeResult,
      CONFIG.CLIENT_PRIVATE_ENCRYPTION_KEY
    );
    logger.debug("Corporate VC Code Challenge API Response (JWS): ", jws);
    let decodedData = await this.securityHelper.verifyJWS(jws, CONFIG.MYINFO_JWKS_URL);
    // successful. return data back to frontend
    logger.debug("Corporate VC Code Challenge Data (JWS Verified): ", JSON.stringify(decodedData));
  ...
  getPersonDataWithToken = async function (accessToken, sessionPopKeyPair) {
    try {
      ...
      let personResult;
      personResult = await this.callCorporateVcAPI(sub, accessToken, sessionPopKeyPair);
