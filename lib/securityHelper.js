...
module.exports.verifyJWS = async (compactJWS, jwksUrl) => {
  ...
  try {
    ...
    logger.error("Error with verifying JWS:", error);
...
module.exports.generateSessionKeyPair = async () => {
  ...
  // let sessionKeyPair = crypto.generateKeyPairSync("rsa", options);
...
module.exports.generateClientAssertion = async (
  url,
  clientId,
  privateSigningKey,
  jktThumbprint
  ...
  try {
    ...
    let jwtToken = await jose.JWS.createSign(
...
module.exports.generateDpop = async (
  url,
  nonce,
  ath,
  method,
  ...
  try {
    ...
    let jwk = (await jose.JWK.asKey(sessionPopKeyPair.publicKey, "pem")).toJSON(
      true
    );
    jwk.use = "sig";
    jwk.alg = "ES256";
...
async function getJwks(jwksUrl) {
  var response = await requestHandler.getHttpsResponse(
    "GET",
    jwksUrl,
    null,
    null,
